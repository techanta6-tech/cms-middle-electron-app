import i18n from 'i18next';
import { initReactI18next } from 'react-i18next';

const resources = {
  vi: {
    translation: {
      app: {
        sidebar: {
          alert_wall: "Cảnh Báo",
          emap: "Bản đồ số",
          connections_monitor: "Giám Sát Sự Kiện",
          devices: "Quản Lý Kết Nối",
          help_docs: "Trợ Giúp & Tài Liệu"
        },
        footer: {
          host_target_ports: "Cổng Host & Đích",
          log_stream_status: "Trạng Thái Luồng Log",
          total_logs_received: "Tổng Logs Nhận",
          save_logs: "Lưu Logs",
          view_system_data: "Xem Dữ Liệu Hệ Thống (DEV)",
          on: "BẬT",
          off: "TẮT",
          connected: "ĐÃ KẾT NỐI",
          disconnected: "ĐÃ NGẮT KẾT NỐI",
          listening: "Đang Lắng Nghe",
          language: "Ngôn Ngữ"
        },
        filter: {
          servers: "Servers",
          no_servers: "Chưa có server nào",
          devices: "Thiết Bị",
          no_devices: "Chưa có thiết bị nào",
          event_types: "Loại Sự Kiện",
          no_event_types: "Chưa có loại sự kiện nào",
          all: "Tất cả (All)",
          filter_logs: "Lọc bản ghi"
        },
        alert_wall: {
          alert: "THÔNG BÁO",
          logs: "Bản ghi",
          logs_queue_empty: "Chưa có thông báo nào",
          see_more_alerts: "Xem thêm thông báo",
          drag_to_assign: "Kéo thả để gán",
          auto_config: "Cấu hình tự động",
          svms_camera_devices: "Camera của SVMS",
          no_svms_devices: "Chưa có Camera của SVMS nào",
          camera_devices: "Camera độc lập",
          no_camera_devices: "Chưa có Camera độc lập nào",
          add_new_camera: "Thêm Camera Mới",
          assigned_to_grid: "Đang hiển thị trên ô: ",
          waiting_data: "Đang chờ dữ liệu...",
          no_incoming: "Chưa có thiết bị",
          grid_settings: "Cài đặt khu vực Camera",
          inc_grid: "Thu nhỏ màn hình (Quản lý nhiều thiết bị)",
          dec_grid: "Phóng to màn hình (Quản lý ít thiết bị)",
          close_settings: "Đóng cài đặt",
          lora_network: "Thiết Bị Cảm Biến",
          no_mqtt_devices: "Chưa có thiết bị cảm biến nào",
          sunell_cameras: "Camera Sunell",
          no_sunell_cameras: "Chưa có Camera Sunell nào",
          lora_devices: "LoRa Devices",
          no_lora_devices: "No LoRa Devices"
        },
        emap: {
          drag_to_pin: "Kéo thả để gắn pin",
          waiting_event: "Chưa có sự kiện",
          move: "Di chuyển",
          delete: "Xóa",
          close: "Đóng",
          move_hint: "Kéo pin tới vị trí mới"
        },
        monitor: {
          input_connections: "Nhận dữ liệu từ...",
          sources_emitting: "nguồn phát",
          output_targets: "Kết nối tới...",
          endpoints_receiving: "số kết nối",
          no_input: "Chưa nhận dữ liệu",
          no_output: "Chưa có kết nối",
          unmapped_devices: "Thiết bị không xác định",
          unregistered_orphan: "Bản ghi không xác định",
          unknown_devices: "Thiết bị không xác định",
          target_endpoint: "Địa chỉ đích",
          logs_sent: "Số bản ghi đã gửi",
          status: "Trạng Thái",
          remove_conn: "BỎ KẾT NỐI",
          devices: "DANH SÁCH THIẾT BỊ",
          total_logs: "TỔNG LOGS",
          disconnected_devices_tooltip: "{{count}} thiết bị mất kết nối",
          camera_default_unlinked: "-- Camera mặc định từ Server (Chưa liên kết Camera) --",
          camera_default_linked: "-- Camera mặc định từ Server ({{name}}) --",
          no_camera_no_snapshot: "-- Không có Camera (Không chụp ảnh) --",
          no_devices_mapped: "Không có thiết bị",
          bound_camera: "Camera đã liên kết",
          default_camera: "Camera mặc định",
          no_camera_disabled: "-- Không có Camera (Không chụp ảnh) --",
          bound_camera_desc: "Sau khi liên kết, khi có cảnh báo từ cảm biến, ứng dụng sẽ chụp ảnh từ camera và đính kèm vào thông báo.",
          camera_default: "-- Mặc định --",
          mqtt_sensors: "Cảm biến",
          alarms: "cảnh báo",
          waiting_events: "Chưa nhận được data từ broker — waiting for events",
          offline: "OFFLINE",
          online: "ONLINE",
          connecting: "CONNECTING",
          connected: "CONNECTED",
          disconnected: "DISCONNECTED",
          error: "ERROR",
          server_id: "Server ID",
          topic: "Topic",
          protocol: "Giao thức",
          log_count: "Số lượng bản ghi",
          camera_id: "ID Camera",
          devices_seen: "Thiết bị đã thấy",
          dev_eui: "Dev EUI",
          profile: "Profile",
          alarm_count: "Số lượng cảnh báo",
          last_seen: "Dữ liệu gần nhất",
          mqtt_ip: "MQTT IP",
          broker: "Broker",
          serial: "Serial",
          server_ip: "Server IP",
          sender_ip: "Sender IP",
          version: "Version",
          location: "Địa điểm",
          type: "Type",
          device_count: "Số lượng thiết bị",
          device_ip: "IP của thiết bị",
          device_type: "Loại thiết bị",
          device_index: "Số thứ tự của thiết bị",
          port: "Cổng số",
          last_log: "Bản ghi gần nhất",
          parent_server: "Server cha",
          this_device: "Thông tin của thiết bị này",
          server_name: "Tên server",
          camera_ip: "IP Camera",
          camera_port: "Cổng camera",
          username: "Username",
          rtsp_url: "RTSP URL",
          handle: "Handle",
          search_logs: "Tìm kiếm logs...",
          all_types: "Tất cả cảnh báo",
          no_logs_found: "Không tìm thấy bản ghi nào",
          log_details: "Chi tiết bản ghi",
          show_more: "Xem thêm",
          recent_logs: "logs gần nhất",
          unmapped_tooltip: "Các logs không có cấu hình Server/Device tương ứng",
          camera_devices_tooltip: "Các camera kết nối độc lập như Sunell và RTSP",
          device_type_tooltip: "Phân loại thiết bị",
          camera_log_total_tooltip: "Tổng logs nhận được từ cameras",
          active_camera_count: "{{count}} camera đang hoạt động",
          error_camera_count: "{{count}} camera lỗi",
          add_new_camera: "Thêm Camera Mới"
        },
        devices: {
          svms_servers: "SVMS Servers",
          no_svms: "Không có SVMS servers nào",
          lora_server: "Lora Servers",
          no_lora: "Không có LoRa servers nào",
          cameras: "Camera độc lập",
          no_cameras: "Không có camera nào",
          sunell_cameras: "Camera Sunell",
          no_sunell_cameras: "Không có Camera Sunell nào",
          select_device: "Chọn một server hoặc thiết bị để xem thông tin chi tiết",
          svms_server: "SVMS Server",
          svms_device: "SVMS Device",
          mqtt_server: "MQTT Server",
          mqtt_device: "MQTT Device",
          camera_device: "Camera độc lập",
          edit: "Sửa",
          confirm_delete_camera: 'Bạn có chắc chắn muốn xóa camera này?',
          confirm_delete_mqtt: 'Bạn có chắc chắn muốn xóa MQTT server này?',
          radar_categories: {
            accident: 'Tai nạn',
            presence: 'Hiện diện',
            vital_signs: 'Sinh tồn',
            enabled_count: 'bật',
            hint: 'Click vào nút gạt bên phải của từng dòng để bật/tắt sự kiện báo động đó một cách độc lập.',
            unassigned: 'Chưa gán (Không chụp ảnh)',
            sub_fall: 'Cảnh báo Té ngã',
            sub_out_of_bed: 'Cảnh báo Rời giường',
            sub_lying: 'Trạng thái Nằm',
            sub_occupied: 'Có người',
            sub_vacant: 'Không có người',
            sub_dwell: 'Cảnh báo Ở quá lâu trong khu vực',
            sub_bradynea: 'Cảnh báo Nhịp thở chậm',
            sub_tachypnea: 'Cảnh báo Nhịp thở nhanh',
            sub_motionless: 'Cảnh báo Bất động',
            other_events: 'Sự kiện khác',
            other_events_hint: 'Nhận tất cả event không thuộc danh sách trên'
          },
          sunell_categories: {
            motion: 'Motion / Chuyển động',
            lpr: 'LPR / Biển số xe',
            face: 'Face / Khuôn mặt',
            iva: 'IVA / Hành vi thông minh',
            system: 'System / Hệ thống',
            subevent_count: 'sub-events',
            hint: 'Tắt category sẽ bỏ qua toàn bộ sub-events thuộc nhóm đó. Click ▶ để xem danh sách sub-event.'
          }
        },
        camera_form: {
          add_camera: "Thêm Camera",
          type: "Loại",
          sunell: "Camera Sunell",
          other_rtsp: "Khác",
          name_optional: "Tên (Tùy chọn)",
          name_placeholder: "Tên gợi nhớ",
          camera_ip: "Camera IP (*)",
          control_port: "Control Port (*)",
          rtsp_port: "RTSP Port (*)",
          username: "Username (*)",
          password: "Password (*)",
          rtsp_optional: "RTSP URL",
          cancel: "Hủy",
          saving: "Đang lưu...",
          save: "Lưu Camera",
          error_fill_all: 'Vui lòng điền đầy đủ các trường bắt buộc (*)',
          error_save_failed: 'Lưu camera thất bại. Vui lòng thử lại.',
          success_save: 'Lưu camera thành công'
        },
        config_system: {
          title: 'Cấu hình kết nối hệ thống',
          operation_mode: 'Chế độ hoạt động',
          system: 'Hệ thống',
          target_ip: 'Địa chỉ IP đích',
          access_port: 'Cổng truy cập',
          cancel: 'Hủy',
          confirm: 'Xác nhận đồng bộ',
          connecting: 'Đang kết nối...',
          note: 'Lưu ý: Thay đổi sẽ khởi tạo lại giao thức socket handshake.'
        },
        add_server: {
          title: 'Thêm kết nối',
          conn_type: 'Loại kết nối',
          op_mode: 'Chế độ hoạt động',
          receive: 'Nhận (Receive)',
          send: 'Gửi (Send)',
          target_ip: 'Địa chỉ IP đích',
          access_port: 'Cổng truy cập',
          protocol: 'Giao thức',
          broker_host: 'Máy chủ Broker',
          port: 'Cổng',
          sub_topic: 'Topic đăng ký',
          use_default: 'Dùng Template Mặc định',
          default_active: '✓ Đã dùng Template',
          cancel: 'Hủy',
          connect_mqtt: 'Kết nối MQTT',
          confirm: 'Xác nhận đồng bộ',
          mqtt_note: 'Client MQTT sẽ kết nối tới broker và đăng ký nhận thông tin từ topic được chỉ định.',
          svms_note: 'Lưu ý: Thay đổi sẽ khởi tạo lại giao thức socket handshake.',
          mqtt_name: 'Tên Server (Tuỳ chọn)',
          mqtt_name_placeholder: 'Tên gợi nhớ',
          tooltip_receive: 'Lắng nghe dữ liệu đẩy về từ một nguồn khác (SVMS/Middle App khác)',
          tooltip_send: 'Đẩy dữ liệu sang một hệ thống CMS hoặc Middle App đích khác',
          error_fill_all: 'Vui lòng nhập đầy đủ thông tin địa chỉ và cổng',
          error_invalid_ip: 'Địa chỉ IP không hợp lệ'
        },
        login: {
          title: 'ANTA 6 CMS DEMO',
          username: 'Tài khoản',
          password: 'Mật khẩu',
          login_btn: 'ĐĂNG NHẬP',
          authenticating: 'ĐANG XÁC THỰC...',
          connection_settings: 'Cài đặt kết nối',
          authorized_personnel: 'Chỉ dành cho nhân viên được ủy quyền',
          host_ip: 'Địa chỉ IP Host',
          port: 'Cổng',
          save_apply: 'Lưu & Áp dụng',
          checking: 'Đang kiểm tra...',
          error: 'Thông tin đăng nhập không hợp lệ hoặc không thể kết nối tới máy chủ'
        },
        log_popup: {
          title: 'Chi tiết Log',
          close: 'Đóng (ESC)',
          event_summary: 'Tóm tắt Sự kiện',
          server_name: "Tên Máy chủ",
          server_id: "ID Máy chủ",
          device_name: "Tên Thiết bị",
          device_ip: "IP Thiết bị",
          device_port: "Cổng Thiết bị",
          device_index: "Chỉ mục Thiết bị",
          device_type: "Loại Thiết bị",
          log_type: "Loại Bản ghi",
          description: "Mô tả",
          source_ip: "IP Nguồn",
          timestamp: "Thời gian",
          unknown_type: "Không rõ loại",
          unknown_status: "Không rõ trạng thái",
          no_media: "KHÔNG CÓ HÌNH ẢNH/VIDEO",
          system_metadata: "THÔNG TIN HỆ THỐNG",
          other_metadata: "THÔNG TIN CHI TIẾT",
          raw_data: "DỮ LIỆU GỐC",
          close_report: "Đóng báo cáo",
          plate_number: "Biển số xe",
          confidence: "Độ tin cậy"
        },
        logtype: {
          motion: 'Phát hiện chuyển động',


          crosswire: 'Hàng rào ảo',
          direction: 'Hướng di chuyển',
          a_motion_has_been_detected: 'SVMS phát hiện chuyển động',

          // SVMS
          svms_ai_alarm_crosswire_all: 'Hàng rào ảo',
          svms_ai_alarm_direction_all: 'Hướng di chuyển',
          svms_ai_alarm_missing_all: 'Không tìm thấy vật thể',
          svms_an_alarm_has_been_detected: 'SVMS phát cảnh báo',
          svms_videoloss: 'Mất tín hiệu camera',
          svms_motion: 'Chuyển động',

          // SVMS event description keys (i18n từ điền bởi project, fallback: 'Thông báo từ SVMS')
          svms_motion_description: 'SVMS phát hiện chuyển động',
          svms_ai_alarm_crosswire_all_description: 'SVMS phát hiện vượt rào',
          svms_ai_alarm_direction_all_description: 'SVMS phát hiện đi sai hướng',
          svms_ai_alarm_missing_all_description: 'SVMS phát hiện vật thể biến mất',
          svms_videoloss_description: 'SVMS mất tín hiệu video',

          // Milesight
          milesight_fall: 'Có người bị ngã',
          milesight_lying: 'Ngã lâu',
          milesight_occupied: 'Có người',
          milesight_vacant: 'Phòng trống',
          milesight_dwell: 'Ở lại quá lâu',
          milesight_motionless: 'Bất động',
          milesight_out_of_bed: 'Rời giường',
          milesight_bradynea: 'Hô hấp chậm',
          milesight_tachypnea: 'Hô hấp nhanh',

          milesight_button: 'Nút bấm khẩn cấp',
          milesight_radar: 'Cảm biến té ngã',
          milesight_button_pressed: 'Nút bấm khẩn cấp',
          milesight_button_pressed_description: 'Có người nhấn nút khẩn cấp',
          button_pressed: 'Nút bấm khẩn cấp',
          short_press: 'Có người nhấn nút khẩn cấp',
          double_press: 'Có người nhấn nút khẩn cấp hai lần',
          long_press: 'Có người nhấn giữ nút khẩn cấp',
          hold_press: 'Có người nhấn giữ nút khẩn cấp',
          very_long_press: 'Có người nhấn giữ nút khẩn cấp rất lâu',
          occupancy: 'Có người trong phòng',

          // Milesight event description keys (i18n từ điền bởi project, fallback: 'Thông báo MQTT')
          milesight_fall_description: 'Có người bị ngã',
          milesight_lying_description: 'Có người ngã quá lâu',
          milesight_occupied_description: 'Có người trong phòng',
          milesight_vacant_description: 'Không có ai trong phòng',
          milesight_dwell_description: 'Có người ở lại quá lâu trong khu vực',
          milesight_motionless_description: 'Bất động bất thường',
          milesight_out_of_bed_description: 'Có người rời khỏi giường quá lâu',
          milesight_bradynea_description: 'Có người hô hấp chậm bất thường',
          milesight_tachypnea_description: 'Có người hô hấp nhanh bất thường',

          // Sunell event 
          sunell_motion_event: 'Sự kiện chuyển động',
          sunell_lpr_event: 'Sự kiện biển số (LPR)',
          sunell_face_event: 'Sự kiện khuôn mặt',
          sunell_alarm_event: 'Sự kiện báo động',
          sunell_tripwire_event: 'Sự kiện hàng rào ảo',
          sunell_perimeter_event: 'Sự kiện xâm nhập vùng cấm',
          sunell_loiter_event: 'Sự kiện lảng vảng',
          sunell_object_left_event: 'Sự kiện bỏ quên đồ vật',
          sunell_object_removed_event: 'Sự kiện mất cắp đồ vật',
          sunell_illegal_parking_event: 'Sự kiện đậu xe trái phép',
          sunell_unknown: 'Sự kiện không xác định',


          // Sunell SDK
          lpr_event: 'Sự kiện biển số',
          face_event: 'Sự kiện khuôn mặt',
          motion_event: 'Sự kiện chuyển động',

          // Aliases / Legacy
          // 'phát_hiện_chuyển_động_(motion)': 'Phát hiện chuyển động (Motion)',
          // 'phát_hiện_biển_số_(lpr)': 'Phát hiện biển số (LPR)',
          // 'phát_hiện_khuôn_mặt_(face)': 'Phát hiện khuôn mặt (Face)',
          iva_event: 'Sự kiện AI',
          // 'phân_tích_ai_(ivs/iva)': 'Phân tích AI (IVS/IVA)',
          // 'Fall Alarm': 'Té ngã',
          fall_alarm: 'Té ngã',
          // 'Out Bed Alarm': 'Rời khỏi giường1',
          out_bed_alarm: 'Rời khỏi giường',
          // 'Dwell Alarm': 'Ở lại quá lâu1',
          dwell_alarm: 'Ở lại quá lâu',
          // 'Dwell time Alarm': 'Ở lại quá lâu3',
          // 'Abnormal Static Alarm': 'Bất động bất thường',
          static_alarm: 'Bất động bất thường',
          // 'Vacant Alarm': 'Phòng trống1',
          vacant_alarm: 'Phòng trống',
          // 'Occupy Alarm': 'Có người',
          occupy_alarm: 'Có người',

          // Sunell IVA
          iva_trip_wire: 'Hàng rào ảo',
          iva_perimeter_intrusion: 'Xâm nhập vùng cấm',
          iva_double_trip_wire: 'Hàng rào ảo kép',
          iva_retrograde: 'Đi ngược chiều',
          iva_smd: 'Phát hiện đối tượng (SMD)',
          iva_occlusion: 'Phân tích che khuất',
          iva_loitering: 'Lảng vảng',
          iva_crowd_loitering: 'Đám đông lảng vảng',
          iva_object_left: 'Bỏ quên đồ vật',
          iva_object_removed: 'Mất cắp đồ vật',
          iva_abnormal_speed: 'Đi quá tốc độ',
          iva_illegal_parking: 'Đậu xe trái phép',
          iva_camera_shift: 'Camera bị dời góc',
          iva_signal_bad: 'Lỗi tín hiệu video AI',

          sunell_iva_trip_wire: 'Hàng rào ảo',
          sunell_iva_perimeter_intrusion: 'Xâm nhập vùng cấm',
          sunell_iva_double_trip_wire: 'Hàng rào ảo kép',
          sunell_iva_retrograde: 'Đi ngược chiều',
          sunell_iva_smd: 'Phát hiện đối tượng (SMD)',
          sunell_iva_occlusion: 'Phân tích che khuất',
          sunell_iva_loitering: 'Lảng vảng',
          sunell_iva_crowd_loitering: 'Đám đông lảng vảng',
          sunell_iva_object_left: 'Bỏ quên đồ vật',
          sunell_iva_object_removed: 'Mất cắp đồ vật',
          sunell_iva_abnormal_speed: 'Đi quá tốc độ',
          sunell_iva_illegal_parking: 'Đậu xe trái phép',
          sunell_iva_camera_shift: 'Camera bị dời góc',
          sunell_iva_signal_bad: 'Lỗi tín hiệu video AI',

          sunell_iva_trip_wire_description: 'Có người đi qua hàng rào ảo',
          sunell_iva_perimeter_intrusion_description: 'Có người xâm nhập vào vùng cấm',
          sunell_iva_double_trip_wire_description: 'Có người đi qua hàng rào ảo kép',
          sunell_iva_retrograde_description: 'Có người đi ngược chiều',
          sunell_iva_smd_description: 'Phát hiện đối tượng',
          sunell_iva_occlusion_description: 'Camera bị che khuất',
          sunell_iva_loitering_description: 'Có người lảng vảng trong khu vực',
          sunell_iva_crowd_loitering_description: 'Có đám đông lảng vảng',
          sunell_iva_object_left_description: 'Có người bỏ quên đồ vật',
          sunell_iva_object_removed_description: 'Có người lấy đi đồ vật',
          sunell_iva_abnormal_speed_description: 'Có người đi quá tốc độ',
          sunell_iva_illegal_parking_description: 'Có người đậu xe trái phép',
          sunell_iva_camera_shift_description: 'Có camera bị dời góc',
          sunell_iva_signal_bad_description: 'Gặp lỗi tín hiệu video AI',

          sunell_lpr_event_description: 'Có biển số xe được phát hiện',
          sunell_motion_event_description: 'Có chuyển động được phát hiện',
          sunell_face_event_description: 'Có khuôn mặt được phát hiện',
          sunell_alarm_event_description: 'Có báo động được kích hoạt',
          sunell_tripwire_event_description: 'Có người đi qua hàng rào ảo',
          sunell_perimeter_event_description: 'Có người xâm nhập vào vùng cấm',
          sunell_loiter_event_description: 'Có người lảng vảng trong khu vực',
          sunell_object_left_event_description: 'Có người bỏ quên đồ vật',
          sunell_object_removed_event_description: 'Có người lấy đi đồ vật',
          sunell_illegal_parking_event_description: 'Có người đậu xe trái phép',
          sunell_unknown_description: 'Có sự kiện không xác định được phát hiện',

          // MQTT Radar/Sensor
          data: 'Dữ liệu Radar',
          raw: 'Dữ liệu thô',
          fall: 'Té ngã',
          out_of_bed: 'Rời khỏi giường',
          dwell: 'Ở lại quá lâu',
          motionless: 'Bất động',
          respiratory_status: 'Trạng thái sinh tồn',
          bradynea: 'Thở chậm',
          tachypnea: 'Thở nhanh',
          lying: 'Đang nằm',
          vacant: 'Phòng trống',
          occupied: 'Có người trong phòng',
          mqtt_fall_alarm: 'Té ngã',
          mqtt_out_bed_alarm: 'Rời khỏi giường',
          mqtt_dwell_alarm: 'Ở lại quá lâu',
          mqtt_static_alarm: 'Bất động bất thường',
          mqtt_vacant_alarm: 'Phòng trống',
          mqtt_occupy_alarm: 'Có người',
          debug_raw: 'Debug Raw',
        },
        mqtt_alarm_type: {
          '0': 'Ngã',
          '1': 'Bất động',
          '2': 'Ở lại lâu',
          '3': 'Rời khỏi giường',
          '4': 'Có người',
          '5': 'Không có người',
          '6': 'Nhịp tim chậm',
          '7': 'Thở nhanh',
          '8': 'Đang nằm',
          'fall': 'Có người bị ngã',
          'motionless': 'Bất động bất thường',
          'dwell': 'Ở trong khu vực quá lâu',
          'out-of-bed': 'Có người rời khỏi giường',
          'out_of_bed': 'Có người rời khỏi giường',
          'occupancy': 'Có người trong phòng',
          'occupied': 'Có người trong phòng',
          'vacant': 'Không có người trong phòng',
          'bradynea': 'Thở chậm bất thường',
          'tachypnea': 'Thở nhanh bất thường',
          'lying': 'Đang nằm',
          'respiratory_status': 'Trạng thái sinh tồn',

          // 'short_press': 'Có người nhấn nút khẩn cấp',
          // 'double_press': 'Có người nhấn nút khẩn cấp hai lần',
          // 'long_press': 'Có người nhấn giữ nút khẩn cấp',

          'debug_raw': 'Debug Raw',
        },
        mqtt_alarm_status: {
          '1': 'Báo động',
          '2': 'Đã giải quyết',
          '3': 'Bỏ qua',
          '4': 'Báo cáo trạng thái',
          'alarm': 'Báo động',
          'resolve': 'Đã giải quyết',
          'ignore': 'Bỏ qua',
          'alarm_ignored': 'Bỏ qua',
          'status_report': 'Báo cáo trạng thái',
          'alarm_triggered': 'Báo động',
          'alarm_canceled': 'Đã giải quyết'
        },
        // ─── Sunell Camera: Alarm System ───
        sunell_main_type: {
          '1': 'Báo động an ninh',
          '4': 'Báo động ổ cứng',
          '5': 'Báo động luồng Video',
          '6': 'Phân tích AI thông minh'
        },
        sunell_alarm: {
          // main_type 1: An ninh cơ bản
          '1/1': 'Báo động I/O',
          '1/2': 'Phát hiện chuyển động',
          '1/3': 'Camera bị che khuất',
          '1/4': 'Mất tín hiệu hình ảnh',
          '1/5': 'Rớt kết nối mạng',
          '1/9': 'Phát hiện thân nhiệt PIR',
          '1/10': 'Báo động cổng I/O NVR',
          // main_type 4: Ổ cứng
          '4/1': 'Ổ cứng bình thường',
          '4/2': 'Lỗi đọc/ghi ổ cứng',
          '4/3': 'Mất kết nối ổ mạng',
          '4/4': 'Ổ cứng đầy',
          '4/5': 'Không có ổ cứng',
          '4/6': 'Dung lượng đạt ngưỡng cảnh báo',
          '4/7': 'Ổ cứng chưa được format',
          '4/8': 'Thiếu dung lượng lưu trữ',
          // main_type 5: Luồng Video
          '5/1': 'Kết nối luồng thành công',
          '5/2': 'Sai tài khoản/mật khẩu luồng',
          '5/3': 'Không có quyền truy cập',
          '5/4': 'Đạt giới hạn số kết nối',
          // main_type 6: AI / IVS
          '6/21': 'Vượt hàng rào ảo',
          '6/22': 'Phát hiện đối tượng di chuyển (SMD)',
          '6/23': 'Phân tích che khuất',
          '6/24': 'Xâm nhập vùng cấm',
          '6/25': 'Vượt hàng rào ảo kép',
          '6/26': 'Lảng vảng',
          '6/27': 'Đám đông lảng vảng',
          '6/28': 'Bỏ quên đồ vật',
          '6/29': 'Mất cắp đồ vật',
          '6/30': 'Đi quá tốc độ',
          '6/31': 'Đi ngược chiều',
          '6/32': 'Đậu xe trái phép',
          '6/33': 'Camera bị dời góc',
          '6/34': 'Lỗi tín hiệu video AI',
          '6/37': 'Nhận diện biển số',
          'unknown': 'Sự kiện chưa xác định'
        },
        // ─── Sunell Camera: AI Detection Target Types ───
        sunell_detect_type: {
          '0': 'Khuôn mặt',
          '2': 'Thân người',
          '3': 'Biển số xe',
          '4': 'Phương tiện',
          'unknown': 'Đối tượng không xác định'
        },
        sunell_alarm_flag: {
          '0': 'Kết thúc',
          '1': 'Đang xảy ra'
        },
        sunell_log_type: {
          'lpr_event': 'Sự kiện biển số (LPR)',
          'motion_event': 'Sự kiện chuyển động',
          'face_event': 'Sự kiện khuôn mặt',
          'alarm_event': 'Sự kiện báo động',
          'tripwire_event': 'Sự kiện hàng rào ảo',
          'perimeter_event': 'Sự kiện xâm nhập vùng cấm',
          'loiter_event': 'Sự kiện lảng vảng',
          'object_left_event': 'Sự kiện bỏ quên đồ vật',
          'object_removed_event': 'Sự kiện mất cắp đồ vật',
          'illegal_parking_event': 'Sự kiện đậu xe trái phép',
          'unknown': 'Sự kiện không xác định'
        }
      }
    }
  },
  en: {
    translation: {
      app: {
        sidebar: {
          alert_wall: "Alert Wall",
          emap: "E-MAP",
          connections_monitor: "Events Monitor",
          devices: "Connections Management",
          help_docs: "Help & Docs"
        },
        footer: {
          host_target_ports: "Host & Target Ports",
          log_stream_status: "Log Stream Status",
          total_logs_received: "Total Logs Received",
          save_logs: "Save Logs",
          view_system_data: "View System Data (DEV)",
          on: "ON",
          off: "OFF",
          connected: "CONNECTED",
          disconnected: "DISCONNECTED",
          listening: "Listening",
          language: "Language"
        },
        filter: {
          servers: "Servers",
          no_servers: "No servers",
          devices: "Devices",
          no_devices: "No devices",
          event_types: "Event Types",
          no_event_types: "No event types",
          all: "All",
          filter_logs: "Filter Logs"
        },
        alert_wall: {
          alert: "ALERT",
          logs: "Logs",
          logs_queue_empty: "Logs queue empty",
          see_more_alerts: "See more alerts",
          drag_to_assign: "Drag to assign",
          auto_config: "Auto Config",
          svms_camera_devices: "SVMS Camera Devices",
          no_svms_devices: "No SVMS Devices",
          camera_devices: "Camera Devices",
          no_camera_devices: "No Camera Devices",
          add_new_camera: "Add New Camera",
          assigned_to_grid: "Assigned to grid: ",
          waiting_data: "Waiting for data stream",
          no_incoming: "No incoming data streams detected",
          grid_settings: "Grid Settings",
          inc_grid: "Zoom Out (Increase Columns)",
          dec_grid: "Zoom In (Decrease Columns)",
          close_settings: "Close Settings",
          lora_network: "Sensor Devices",
          no_mqtt_devices: "No sensor devices",
          sunell_cameras: "Sunell Cameras",
          no_sunell_cameras: "No Sunell Cameras",
          lora_devices: "LoRa Devices",
          no_lora_devices: "No LoRa Devices"
        },
        emap: {
          drag_to_pin: "Drag to pin",
          waiting_event: "No event yet",
          move: "Move",
          delete: "Delete",
          close: "Close",
          move_hint: "Drag the pin to a new position"
        },
        monitor: {
          input_connections: "Input Connections",
          sources_emitting: "sources emitting",
          output_targets: "Output Targets",
          endpoints_receiving: "endpoints receiving",
          no_input: "No input connections",
          no_output: "No output targets configured",
          unmapped_devices: "Unmapped / External Devices",
          unregistered_orphan: "UNREGISTERED ORPHAN LOGS",
          unknown_devices: "UNKNOWN DEVICES",
          target_endpoint: "Target Endpoint",
          logs_sent: "Logs Sent",
          status: "Status",
          remove_conn: "REMOVE CONNECTION",
          devices: "DEVICES",
          total_logs: "TOTAL LOGS",
          disconnected_devices_tooltip: "{{count}} disconnected devices",
          camera_default_unlinked: "-- Default Camera from Server (Unlinked Camera) --",
          camera_default_linked: "-- Default Camera from Server ({{name}}) --",
          no_camera_no_snapshot: "-- No Camera (No Snapshot) --",
          no_devices_mapped: "No devices mapped from this server",
          bound_camera: "Bound Camera",
          default_camera: "Default Camera",
          no_camera_disabled: "-- No Camera (Disabled Snapshot) --",
          bound_camera_desc: "If bound, when an MQTT alarm triggers, the server will try to extract a snapshot from this camera.",
          camera_default: "-- Default --",
          mqtt_sensors: "MQTT Sensors",
          alarms: "alarms",
          waiting_events: "No data from broker — waiting for events",
          offline: "OFFLINE",
          online: "ONLINE",
          connecting: "CONNECTING",
          connected: "CONNECTED",
          disconnected: "DISCONNECTED",
          error: "ERROR",
          server_id: "Server ID",
          topic: "Topic",
          protocol: "Protocol",
          log_count: "Log Count",
          camera_id: "Camera ID",
          devices_seen: "Devices Seen",
          dev_eui: "Dev EUI",
          profile: "Profile",
          alarm_count: "Alarm Count",
          last_seen: "Last Seen",
          mqtt_ip: "MQTT IP",
          broker: "Broker",
          serial: "Serial",
          server_ip: "Server IP",
          sender_ip: "Sender IP",
          version: "Version",
          location: "Location",
          type: "Type",
          device_count: "Device Count",
          device_ip: "Device IP",
          device_type: "Device Type",
          device_index: "Device Index",
          port: "Port",
          last_log: "Last Log",
          parent_server: "Parent Server",
          this_device: "This Device Information",
          server_name: "Server Name",
          camera_ip: "Camera IP",
          camera_port: "Camera Port",
          username: "Username",
          rtsp_url: "RTSP URL",
          handle: "Handle",
          search_logs: "Search logs...",
          all_types: "All alert types",
          no_logs_found: "No logs found",
          log_details: "Log details",
          show_more: "Show more",
          recent_logs: "recent logs",
          unmapped_tooltip: "Logs without corresponding Server/Device configuration",
          camera_devices_tooltip: "Independently connected cameras like Sunell and RTSP",
          device_type_tooltip: "Device classification",
          camera_log_total_tooltip: "Total logs received from cameras",
          active_camera_count: "{{count}} active cameras",
          error_camera_count: "{{count}} error cameras",
          add_new_camera: "Add New Camera"
        },
        devices: {
          svms_servers: "SVMS Servers",
          no_svms: "No SVMS servers",
          lora_server: "Lora Servers",
          no_lora: "No LoRa servers",
          cameras: "Independent Cameras",
          no_cameras: "No cameras found",
          sunell_cameras: "Sunell Cameras",
          no_sunell_cameras: "No Sunell cameras found",
          select_device: "Select device to view details",
          svms_server: "SVMS Server",
          svms_device: "SVMS Device",
          mqtt_server: "MQTT Server",
          mqtt_device: "MQTT Device",
          camera_device: "Camera Device",
          edit: "Edit",
          confirm_delete_camera: 'Are you sure you want to delete this camera?',
          confirm_delete_mqtt: 'Are you sure you want to delete this MQTT server?',
          radar_categories: {
            accident: 'Accident',
            presence: 'Presence & Stay',
            vital_signs: 'Vital Signs',
            enabled_count: 'enabled',
            hint: 'Click the toggle on the right of each row to enable/disable that specific alarm event.',
            unassigned: 'Unassigned (No snapshot)',
            sub_fall: 'Fall Alarm',
            sub_out_of_bed: 'Out of Bed Alarm',
            sub_lying: 'Lying State',
            sub_occupied: 'Occupied',
            sub_vacant: 'Vacant',
            sub_dwell: 'Dwell / Stay Alarm',
            sub_bradynea: 'Bradynea Alarm',
            sub_tachypnea: 'Tachypnea Alarm',
            sub_motionless: 'Abnormal Static Alarm',
            other_events: 'Other Events',
            other_events_hint: 'Accept any event type not listed above'
          },
          sunell_categories: {
            motion: 'Motion Detection',
            lpr: 'License Plate Recognition',
            face: 'Face Detection',
            iva: 'Intelligent Behavior (IVA)',
            system: 'System Events',
            subevent_count: 'sub-events',
            hint: 'Disabling a category will ignore all its sub-events. Click ▶ to view sub-events.'
          }
        },
        camera_form: {
          add_camera: "Add Camera",
          type: "Type",
          sunell: "Sunell Camera",
          other_rtsp: "Other (RTSP Only)",
          name_optional: "Name (Optional)",
          name_placeholder: "Friendly name",
          camera_ip: "Camera IP (*)",
          control_port: "Control Port (*)",
          rtsp_port: "RTSP Port (*)",
          username: "Username (*)",
          password: "Password (*)",
          rtsp_optional: "RTSP URL (Optional)",
          cancel: "Cancel",
          saving: "Saving...",
          save: "Save Camera",
          error_fill_all: 'Please fill in all required fields (*)',
          error_save_failed: 'Failed to save camera. Please try again.',
          success_save: 'Camera saved successfully'
        },
        // Settings & Modals
        config_system: {
          title: 'Config System Connection',
          operation_mode: 'Operation Mode',
          system: 'System',
          target_ip: 'Target IP Address',
          access_port: 'Access Port',
          cancel: 'Cancel',
          confirm: 'Confirm Sync',
          connecting: 'Connecting...',
          note: 'Note: Changes will re-initialize the socket handshake protocol.'
        },
        add_server: {
          title: 'Add Connection',
          conn_type: 'Connection Type',
          op_mode: 'Operation Mode',
          receive: 'Receive',
          send: 'Send',
          target_ip: 'Target IP Address',
          access_port: 'Access Port',
          protocol: 'Protocol',
          broker_host: 'Broker Host',
          port: 'Port',
          sub_topic: 'Subscribe Topic',
          use_default: 'Use Default Template',
          default_active: '✓ Default Template',
          cancel: 'Cancel',
          connect_mqtt: 'Connect MQTT',
          confirm: 'Confirm Sync',
          mqtt_note: 'MQTT client will connect to the broker and subscribe to the specified topic.',
          svms_note: 'Note: Changes will re-initialize the socket handshake protocol.',
          mqtt_name: 'Server Name (Optional)',
          mqtt_name_placeholder: 'Friendly name',
          tooltip_receive: 'Listen for incoming data pushed from another source (SVMS/Other Middle App)',
          tooltip_send: 'Push data to another CMS or destination Middle App system',
          error_fill_all: 'Please fill in both IP address and port',
          error_invalid_ip: 'Invalid IP address'
        },
        login: {
          title: 'ANTA 6 CMS DEMO',
          username: 'Username',
          password: 'Password',
          login_btn: 'LOGIN',
          authenticating: 'AUTHENTICATING...',
          connection_settings: 'Connection Settings',
          authorized_personnel: 'Authorized Personnel Only',
          host_ip: 'Host IP',
          port: 'Port',
          save_apply: 'Save & Apply',
          checking: 'Checking...',
          error: 'Invalid credentials or server unreachable'
        },
        log_popup: {
          title: 'Log Detail',
          close: 'Close (ESC)',
          event_summary: 'Event Summary',
          server_name: "Server Name",
          server_id: "Server ID",
          device_name: "Device Name",
          device_ip: "Device IP",
          device_port: "Device Port",
          device_index: "Device Index",
          device_type: "Device Type",
          log_type: "Log Type",
          description: "Description",
          source_ip: "Source IP",
          timestamp: "Timestamp",
          unknown_type: "Unknown Type",
          unknown_status: "Unknown Status",
          no_media: "NO MEDIA AVAILABLE",
          system_metadata: "SYSTEM METADATA",
          other_metadata: "DETAILED METADATA",
          raw_data: "RAW DATA",
          close_report: "Close Report",
          plate_number: "Plate Number",
          confidence: "Confidence"
        },
        logtype: {

          motion: 'Motion detected',

          // SVMS (display names)
          crosswire: 'Crosswire',
          ai_alarm_crosswire_all: 'Crosswire',
          direction: 'Direction',
          ai_alarm_direction_all: 'Direction',
          ai_alarm_missing_all: 'Missing Object',
          an_alarm_has_been_detected: 'SVMS Alarm',
          videoloss: 'Video Signal Loss',
          svms_ai_alarm_crosswire_all: 'Crosswire',
          svms_ai_alarm_direction_all: 'Direction',
          svms_ai_alarm_missing_all: 'Missing Object',
          svms_an_alarm_has_been_detected: 'SVMS Alarm',
          svms_videoloss: 'Video Signal Loss',
          svms_motion: 'Motion',

          // SVMS event description keys (filled by project, fallback: 'SVMS Alert')
          svms_motion_description: 'SVMS detect motion',
          svms_ai_alarm_crosswire_all_description: 'SVMS detect crossing wire',
          svms_ai_alarm_direction_all_description: 'SVMS detect object went wrong direction',
          svms_ai_alarm_missing_all_description: 'SVMS detect missing object',
          svms_videoloss_description: 'SVMS camera lose connection',

          // Milesight (display names)
          milesight_fall: 'Fall',
          milesight_lying: 'Lying',
          milesight_occupied: 'Occupied',
          milesight_vacant: 'Vacant',
          milesight_dwell: 'Dwell',
          milesight_motionless: 'Motionless',
          milesight_out_of_bed: 'Out of Bed',
          milesight_bradynea: 'Bradynea',
          milesight_tachypnea: 'Tachypnea',

          milesight_button: 'Button',
          milesight_radar: 'Radar',
          milesight_button_pressed: 'Emergency button',
          milesight_button_pressed_description: 'Emergency button pressed',
          button_pressed: 'Emergency button',
          short_press: 'Emergency button pressed',
          double_press: 'Emergency button double pressed',
          long_press: 'Emergency button held',
          hold_press: 'Emergency button held',
          very_long_press: 'Emergency button held very long',
          occupancy: 'Occupancy',

          // // Milesight event description keys (filled by project, fallback: 'MQTT Alert')
          milesight_fall_description: 'Person fallen',
          milesight_lying_description: 'Person lying down too long',
          milesight_occupied_description: 'Room occupied',
          milesight_vacant_description: 'Room vacant',
          milesight_dwell_description: 'Person dwelling too long in area',
          milesight_motionless_description: 'Abnormal motionlessness',
          milesight_out_of_bed_description: 'Out of bed too long',
          milesight_bradynea_description: 'Abnormal bradynea detected',
          milesight_tachypnea_description: 'Abnormal tachypnea detected',

          // Sunell (display names)
          sunell_motion_event: 'Motion Event',
          sunell_lpr_event: 'License Plate Event (LPR)',
          sunell_face_event: 'Face Event',
          sunell_alarm_event: 'Alarm Event',
          sunell_tripwire_event: 'Tripwire Event',
          sunell_perimeter_event: 'Perimeter Intrusion Event',
          sunell_loiter_event: 'Loitering Event',
          sunell_object_left_event: 'Object Left Behind Event',
          sunell_object_removed_event: 'Object Removed Event',
          sunell_illegal_parking_event: 'Illegal Parking Event',
          sunell_unknown: 'Unknown Event',

          // Sunell IVA (display names with prefix)
          sunell_iva_trip_wire: 'Tripwire',
          sunell_iva_perimeter_intrusion: 'Perimeter Intrusion',
          sunell_iva_double_trip_wire: 'Double Tripwire',
          sunell_iva_retrograde: 'Wrong Way / Retrograde',
          sunell_iva_smd: 'Smart Motion Detection (SMD)',
          sunell_iva_occlusion: 'Occlusion Detection',
          sunell_iva_loitering: 'Loitering',
          sunell_iva_crowd_loitering: 'Crowd Loitering',
          sunell_iva_object_left: 'Object Left Behind',
          sunell_iva_object_removed: 'Object Removed',
          sunell_iva_abnormal_speed: 'Abnormal Speed',
          sunell_iva_illegal_parking: 'Illegal Parking',
          sunell_iva_camera_shift: 'Camera Tampered',
          sunell_iva_signal_bad: 'Video Signal Abnormal',

          // Sunell event description keys
          sunell_lpr_event_description: 'A license plate has been detected',
          sunell_motion_event_description: 'Motion has been detected',
          sunell_face_event_description: 'A face has been detected',
          sunell_alarm_event_description: 'An alarm has been triggered',
          sunell_tripwire_event_description: 'A person crossed the tripwire',
          sunell_perimeter_event_description: 'A person entered a restricted area',
          sunell_loiter_event_description: 'A person is loitering in the area',
          sunell_object_left_event_description: 'An object has been left behind',
          sunell_object_removed_event_description: 'An object has been removed',
          sunell_illegal_parking_event_description: 'Illegal parking detected',
          sunell_unknown_description: 'An unknown event has been detected',
          sunell_iva_trip_wire_description: 'A person crossed the tripwire',
          sunell_iva_perimeter_intrusion_description: 'A person entered a restricted area',
          sunell_iva_double_trip_wire_description: 'A person crossed the double tripwire',
          sunell_iva_retrograde_description: 'A person is moving the wrong way',
          sunell_iva_smd_description: 'Moving object detected',
          sunell_iva_occlusion_description: 'Camera is being blocked',
          sunell_iva_loitering_description: 'A person is loitering in the area',
          sunell_iva_crowd_loitering_description: 'A crowd is loitering',
          sunell_iva_object_left_description: 'An object has been left behind',
          sunell_iva_object_removed_description: 'An object has been removed',
          sunell_iva_abnormal_speed_description: 'A person is moving at abnormal speed',
          sunell_iva_illegal_parking_description: 'Illegal parking detected',
          sunell_iva_camera_shift_description: 'Camera has been tampered',
          sunell_iva_signal_bad_description: 'Video signal error detected',

          // Sunell SDK
          lpr_event: 'LPR Event',
          face_event: 'Face Event',
          motion_event: 'Motion Event',

          // Aliases / Legacy
          a_motion_has_been_detected: 'A motion has been detected',
          'phát_hiện_chuyển_động_(motion)': 'Motion Detection (Motion)',
          'phát_hiện_biển_số_(lpr)': 'License Plate Recognition (LPR)',
          'phát_hiện_khuôn_mặt_(face)': 'Face Detection (Face)',
          iva_event: 'AI Event',
          'phân_tích_ai_(ivs/iva)': 'AI Analysis (IVS/IVA)',
          'Fall Alarm': 'Fall',
          fall_alarm: 'Fall',
          'Out Bed Alarm': 'Out of Bed',
          out_bed_alarm: 'Out of Bed',
          'Dwell Alarm': 'Dwell Time',
          dwell_alarm: 'Dwell Time',
          'Dwell time Alarm': 'Dwell Time',
          'Abnormal Static Alarm': 'Abnormal Static',
          static_alarm: 'Abnormal Static',
          'Vacant Alarm': 'Vacant',
          vacant_alarm: 'Vacant',
          'Occupy Alarm': 'Occupied',
          occupy_alarm: 'Occupied',


          // Sunell IVA
          iva_trip_wire: 'Tripwire',
          iva_perimeter_intrusion: 'Perimeter Intrusion',
          iva_double_trip_wire: 'Double Tripwire',
          iva_retrograde: 'Retrograde',
          iva_smd: 'Smart Motion Detection (SMD)',
          iva_occlusion: 'Occlusion Detection',
          iva_loitering: 'Loitering',
          iva_crowd_loitering: 'Crowd Loitering',
          iva_object_left: 'Object Left Behind',
          iva_object_removed: 'Object Removed',
          iva_abnormal_speed: 'Abnormal Speed',
          iva_illegal_parking: 'Illegal Parking',
          iva_camera_shift: 'Camera Tampered',
          iva_signal_bad: 'Video Signal Abnormal',
          // MQTT Radar/Sensor
          data: 'Radar Data',
          raw: 'Raw Data',
          mqtt_fall_alarm: 'Fall',
          mqtt_out_bed_alarm: 'Out of Bed',
          mqtt_dwell_alarm: 'Dwell Time',
          mqtt_static_alarm: 'Abnormal Static',
          mqtt_vacant_alarm: 'Vacant',
          mqtt_occupy_alarm: 'Occupied',
          dwell: 'Dwell Time',
          occupied: 'Occupied',
          vacant: 'Vacant',
          debug_raw: 'Debug Raw',
          out_of_bed: 'Out of Bed',
          bradynea: 'Bradynea',
          tachypnea: 'Tachypnea',
        },
        mqtt_alarm_type: {
          '0': 'Fall',
          '1': 'Motionless',
          '2': 'Dwell',
          '3': 'Out-of-bed',
          '4': 'Occupancy',
          '5': 'Vacant',
          '6': 'bradynea',
          '7': 'Tachypnea',
          '8': 'Lying',
          'fall': 'Fall',
          'motionless': 'Motionless',
          'dwell': 'Dwell',
          'out-of-bed': 'Out of Bed',
          'out_of_bed': 'Out of Bed',
          'occupancy': 'Occupancy',
          'occupied': 'Occupied',
          'vacant': 'Vacant',
          'bradynea': 'Bradynea',
          'tachypnea': 'Tachypnea',
          'lying': 'Lying',
          'respiratory_status': 'Respiratory Status',
          'debug_raw': 'Debug Raw',
        },
        mqtt_alarm_status: {
          '1': 'Alarm',
          '2': 'Resolve',
          '3': 'Ignore',
          '4': 'Status report',
          'alarm': 'Alarm',
          'resolve': 'Resolve',
          'ignore': 'Ignore',
          'alarm_ignored': 'Ignore',
          'status_report': 'Status report',
          'alarm_triggered': 'Alarm',
          'alarm_canceled': 'Resolve',
        },
        // ─── Sunell Camera: Alarm System ───
        sunell_main_type: {
          '1': 'Safety Alarm',
          '4': 'Disk Alarm',
          '5': 'Video Stream Alarm',
          '6': 'AI Intelligent Analysis'
        },
        sunell_alarm: {
          // main_type 1: Safety
          '1/1': 'I/O Alarm',
          '1/2': 'Motion Detection',
          '1/3': 'Camera Blocked',
          '1/4': 'Video Signal Loss',
          '1/5': 'Network Disconnection',
          '1/9': 'PIR Thermal Detection',
          '1/10': 'NVR Channel I/O Alarm',
          // main_type 4: Disk
          '4/1': 'Disk Status OK',
          '4/2': 'Disk Read/Write Error',
          '4/3': 'Network Disk Connection Failed',
          '4/4': 'Disk Full',
          '4/5': 'Disk Not Found',
          '4/6': 'Storage Threshold Reached',
          '4/7': 'Disk Not Formatted',
          '4/8': 'Insufficient Storage Space',
          // main_type 5: Video Stream
          '5/1': 'Stream Connected',
          '5/2': 'Invalid Credentials',
          '5/3': 'Access Denied',
          '5/4': 'Max Connections Reached',
          // main_type 6: AI / IVS
          '6/21': 'Trip Wire Detection',
          '6/22': 'Smart Motion Detection (SMD)',
          '6/23': 'Occlusion Detection',
          '6/24': 'Perimeter Intrusion',
          '6/25': 'Double Trip Wire',
          '6/26': 'Loitering',
          '6/27': 'Crowd Loitering',
          '6/28': 'Object Left Behind',
          '6/29': 'Object Removed',
          '6/30': 'Abnormal Speed',
          '6/31': 'Wrong Way / Retrograde',
          '6/32': 'Illegal Parking',
          '6/33': 'Camera Tampered',
          '6/34': 'Video Signal Abnormal',
          '6/37': 'License Plate Recognition',
          'unknown': 'Unidentified Event'
        },
        // ─── Sunell Camera: AI Detection Target Types ───
        sunell_detect_type: {
          '0': 'Face',
          '2': 'Person (Body)',
          '3': 'License Plate',
          '4': 'Vehicle',
          'unknown': 'Unknown Object'
        },
        sunell_alarm_flag: {
          '0': 'Ended',
          '1': 'Active'
        },
        sunell_log_type: {
          'lpr_event': 'License Plate Event (LPR)',
          'motion_event': 'Motion Event',
          'face_event': 'Face Recognition Event',
          'alarm_event': 'Alarm Event',
          'tripwire_event': 'Trip Wire Event',
          'perimeter_event': 'Perimeter Intrusion Event',
          'loiter_event': 'Loitering Event',
          'object_left_event': 'Object Left Behind Event',
          'object_removed_event': 'Object Removed Event',
          'illegal_parking_event': 'Illegal Parking Event',
          'unknown': 'Unknown Event'
        }
      }
    }
  }
};

i18n
  .use(initReactI18next)
  .init({
    resources,
    lng: 'vi', // default language
    fallbackLng: 'vi',
    interpolation: {
      escapeValue: false // react already safes from xss
    }
  });

export default i18n;
