...
import { AlertWall } from "./tabs/Alert/AlertWall";
import { DevicesManager } from "./tabs/Devices/DevicesManager";
import { authApi } from "../api/authApi";
import { EMap } from "./tabs/EMap/EMap";
import { DeviceDraggablePanel } from "./components/DeviceDraggablePanel";

type DashboardMqttDevice = Omit<Partial<MqttDevice>, "deviceInfo"> & {
  devEui?: string;
  deviceName?: string;
  deviceProfileName?: string;
  alarmCount?: number;
  deviceInfo?: MqttDevice["deviceInfo"] & { deviceNickname?: string };
};
...
function LogFilter({
  ...
  mqttServers?: any[];
  mqttDevicesByServer?: Record<string, DashboardMqttDevice[]>;
  cameraDevices?: any[];
      ...
      if (!group) {
        ...
        for (const [groupName, members] of Object.entries(fallbackGroups)) {
          if (
            members.some(
              (m) =>
                m.toLowerCase() === searchType ||
                m.toLowerCase().replace(/\./g, "_") === searchType,
            )
          ) {
            group = groupName;
            break;
  ...
  const isAllChecked =
    eventTypes.length > 0 &&
    eventTypes.every((item) => selectedEventTypes.includes(item.event_type));
  ...
  const renderIndividualEventTypeButton = (item: EventTypeItem) => {
      ...
      switch (log_source) {
        case "svms": {
          ...
          if (displayFilterType === `app.logtype.svms_${normalizedType}`) {
            displayFilterType = t(`app.logtype.${normalizedType}`, {
              defaultValue: lt,
            });
          }
          break;
        ...
        default:
          displayFilterType = t(
            `app.logtype.${lt.toLowerCase().replace(/ /g, "_").replace(/\./g, "_")}`,
            { defaultValue: lt },
          );
      }
    } else {
    ...
    return (
      ...
      >
        <div
          className={`w-3 h-3 rounded-sm border-[1.5px] flex items-center justify-center shrink-0 transition-colors ${
            checked ? "bg-warning border-warning" : "border-outline-variant"
          }`}
        >
  ...
  const deviceList = useMemo(() => {
    ...
    const svmsDevs = Object.values(devices)
      .flatMap((serverData) =>
    ...
    const mqttDevs = Object.entries(mqttDevicesByServer || {})
      .flatMap(([serverId, devs]) => {
        return devs.map((d) => ({
          name:
            d.deviceNickname ||
            d.deviceInfo?.deviceNickname ||
            d.deviceName ||
            d.deviceProfileName ||
            d.devEui ||
            "MQTT Device",
  ...
  const getGroupLabel = useCallback(
    (groupKey: string) => {
      ...
      const srv = Object.values(servers).find(
        (s) => s.id === groupKey || s.serial === groupKey,
      );
  ...
  const groupList = useMemo(() => {
    return Object.keys(groupedDevices).map((groupKey) => {
      ...
      if (groupKey.startsWith("SUNELL-LOCAL")) {
        ...
        (mqttGroups || []).some((g) => g.id === groupKey) ||
        groupKey.startsWith("mqtt-")
      ) {
        type = "MQTT";
      }
      return {
  ...
  const activeCount =
    selectedGroups.size + selectedDevices.size + selectedEventTypes.length;
  ...
  return (
    <div
      ref={ref}
      className="app-log-filter flex items-center p-1 cursor-pointer transition-all duration-200 group"
    >
      {/* <button className='absolute bottom-3 right-3' onClick={() => console.log(deviceList)}>TEST HERE CLICK ME</button> */}
      <button
        onClick={() => setOpen((v) => !v)}
        className={`app-log-filter-btn flex items-center gap-1.5 px-2 py-1 rounded-md transition-all duration-200 group border ${
          activeCount > 0
          ...
          <div className="px-2 pt-2 pb-0.5">
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[12px] font-black uppercase tracking-widest text-tertiary">
                  {t("app.filter.devices")}
                </span>
              </div>
              <ChevronDown
                className={`w-3.5 h-3.5 text-tertiary/50 transition-transform duration-300 ${showDevices ? "rotate-180" : ""}`}
              />
            </button>
            {showDevices &&
              (deviceList.length === 0 ? (
                <p className="text-[11px] text-on-surface-variant/40 py-0.5 pl-1">
                  {t("app.filter.no_devices")}
                </p>
              ) : (
                <div className="flex flex-col gap-1 max-h-36 overflow-y-auto custom-scrollbar pr-1 animate-in fade-in slide-in-from-top-1 duration-200 cursor-pointer">
                  <button
                    onClick={() => {
                      if (isAllDevicesChecked) {
                        ...
                        const allKeys = deviceList.map(
                          (dev) =>
                            `${dev.serverId}_${dev.ip}_${dev.originalName || dev.name}`,
                        );
                  ...
                  {Object.entries(groupedDevices).map(([serverKey, devs]) => {
                    ...
                    return (
                      <div
                        key={serverKey}
                        id={`device-group-${serverKey}`}
                        className="flex flex-col gap-[1px] border-b border-outline-variant/5 pb-1 mb-0.5 last:border-0 last:pb-0 last:mb-0"
                      >
                        <button
                          onClick={() => {
                            const groupDevKeys = devs.map(
                              (d) =>
                                `${d.serverId}_${d.ip}_${d.originalName || d.name}`,
                            );
                            const allChecked = groupDevKeys.every(
                              (k: string) =>
                                allDevicesSelected || selectedDevices.has(k),
                            );
                            if (allChecked) {
                              // deselect all in group
                              groupDevKeys.forEach(
                                (k: string) =>
                                  selectedDevices.has(k) && onToggleDevice(k),
                              );
                            } else {
                              // select all in group that aren't already selected
                              groupDevKeys.forEach((k: string) => {
                                if (
                                  !allDevicesSelected &&
                                  !selectedDevices.has(k)
                                )
                                  onToggleDevice(k);
                              });
                            }
                        ...
                        {devs.map((dev) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-[8px] font-mono text-on-surface/90 ml-auto shrink-0">
                                {dev.type.charAt(0).toUpperCase() +
                                  dev.type.slice(1)}
                              </span>
                            </button>
                          );
          ...
          <div className="px-2 pt-2 pb-2">
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[12px] font-black uppercase tracking-widest text-warning">
                  {t("app.filter.event_types")}
                </span>
              </div>
              <ChevronDown
                className={`w-3.5 h-3.5 text-warning/50 transition-transform duration-300 ${showEventTypes ? "rotate-180" : ""}`}
              />
            </button>
            {showEventTypes &&
              (eventTypes.length === 0 ? (
                <p className="text-[11px] text-on-surface-variant/40 py-0.5 pl-1">
                  {t("app.filter.no_event_types")}
                </p>
              ) : (
                  ...
                  <div className="flex flex-col gap-1 pt-1">
                    {filterItems.map((filterItem) => {
                      if (filterItem.type === "group") {
                        ...
                        const checked = members.every((m) =>
                          selectedEventTypes.includes(m.event_type),
                        );
                        const label = t(`app.logtype.${groupName}`, {
                          defaultValue: groupName.toUpperCase(),
                        });
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-[10px] font-bold text-on-surface truncate uppercase">
                              {label}
                            </span>
                          </button>
                        );
...
export function Dashboard() {
  ...
  const setGrids = useCallback(
    (updater: any) => {
      const newGrids =
        typeof updater === "function" ? updater(gridLayout.grids) : updater;
  ...
  const setGridCols = useCallback(
    (updater: any) => {
      const newCols =
        typeof updater === "function" ? updater(gridLayout.gridCols) : updater;
  ...
  const gridHasDevice = useCallback(
    (grid: any, device: any) => {
      return getGridDevices(grid).some(
        (item: any) =>
  ...
  useEffect(() => {
    if (groupList.length > 0) {
      const newGroups = groupList.filter(
        (g) => !seenGroupsRef.current.has(g.id),
      );
  ...
  const [mainTab, setMainTab] = useState<
    "alert" | "emap" | "connections" | "devices" | "traffic" | "livewall"
  >("emap");
  ...
  const eMapKnownDevices = useMemo(() => {
    ...
    const sunellDevices = cameraDevices
      .filter((cam) => cam.type === "sunell")
      .map((cam) => ({
  ...
  const filteredEMapKnownDevices = useMemo(() => {
    ...
    return eMapKnownDevices.filter((device) => {
      ...
      const devGroupId =
        matched.serverId === "SUNELL-LOCAL"
          ? `SUNELL-LOCAL_${matched.type || "sunell"}`
          : matched.serverId;
    ...
    (id: string) => {
      if (allGroupsSelected) {
        ...
        if (allDevicesSelected) {
          ...
          deviceList.forEach((dev) => {
            const devGroupId =
              dev.serverId === "SUNELL-LOCAL"
                ? `SUNELL-LOCAL_${dev.type || "sunell"}`
                : dev.serverId;
            if (devGroupId !== id) {
              const key = `${dev.serverId}_${dev.ip}_${dev.originalName || dev.name}`;
  ...
  const displayLogs = useMemo(() => {
    if (
      (!allGroupsSelected && selectedGroups.size === 0) ||
      (!allDevicesSelected && selectedDevices.size === 0)
    )
      return [];
    return filteredLogs.filter((log) => {
      ...
      if (log.log_source === "sunell-camera") {
        const isLinkedToSelectedRadar =
          allDevicesSelected ||
          Array.from(selectedDevices).some((selectedKey) => {
        ...
        const isLinkedToSelectedServer =
          allGroupsSelected ||
          Array.from(selectedGroups).some((selectedGroupId) => {
            return deviceCameraLinks.some((link) => {
              return (
                link.cameraId === log.device_info.id &&
                (link.groupId || link.mqttServerId) === selectedGroupId
              );
            });
          });
  ...
  return (
    <div className="app-dashboard-root flex flex-col h-screen overflow-hidden bg-background text-on-surface font-sans selection:bg-primary/30 antialiased">
      <main
        className={`app-dashboard-main flex-1 overflow-hidden ${isNarrow ? "flex flex-col" : (mainTab === "alert" || mainTab === "emap" || mainTab === "livewall") && !isAlertWallFullscreen ? "grid grid-cols-4 gap-0" : "flex"}`}
      >
        {/* Main Section */}
        <div
          className={`app-dashboard-left-section overflow-hidden bg-background border-outline-variant/20 ${isNarrow ? "flex-1 border-b" : (mainTab === "alert" || mainTab === "emap" || mainTab === "livewall") && !isAlertWallFullscreen ? "col-span-3 grid grid-rows-[1fr] h-full border-r" : "flex-1 h-full"}`}
        >
          <div className="flex flex-col overflow-hidden h-full">
            {!isAlertWallFullscreen && (
              <div
                className={`appTabsBar flex items-center border-b border-outline-variant/10 shrink-0 ${isNarrow ? "" : "px-6 gap-4"}`}
              >
                ...
                >
                  <MapPinned
                    className={`w-5 h-5 ${mainTab === "emap" ? "text-primary" : "text-on-surface"}`}
                  />
                  <h2
                    className={`text-[10px] font-bold tracking-[0.2em] uppercase ${mainTab === "emap" ? "text-primary" : "text-on-surface"}`}
                  >
                    {t("app.sidebar.emap")}
                  </h2>
                </button>
                {/* Nút chuyển sang màn hình Alert Wall */}
                <button
                ...
                >
                  <AlertCircle
                    className={`w-5 h-5 ${mainTab === "alert" ? "text-primary" : "text-on-surface"}`}
                  />
                  <h2
                    className={`text-[10px] font-bold tracking-[0.2em] uppercase ${mainTab === "alert" ? "text-primary" : "text-on-surface"}`}
                  >
                    {t("app.sidebar.alert_wall")}
                  </h2>
                </button>
                ...
                >
                  <Cpu
                    className={`w-5 h-5 ${mainTab === "devices" ? "text-primary" : "text-on-surface"}`}
                  />
                  <h2
                    className={`text-[10px] font-bold tracking-[0.2em] uppercase ${mainTab === "devices" ? "text-primary" : "text-on-surface"}`}
                  >
                    {t("app.sidebar.devices")}
                  </h2>
                </button>
              </div>
            )}
            {mainTab === "alert" && (
              <AlertWall
                logs={displayLogs}
                cameras={Object.values(devices).flatMap(
                  (server) => server || [],
                )}
        ...
        {(mainTab === "alert" ||
          ...
          !isAlertWallFullscreen && (
            <aside
              className={`alert-wall-right-section bg-surface-container-lowest flex flex-col overflow-hidden shadow-2xl z-10 transition-transform duration-300 ${
                isNarrow
            ...
            >
              {mainTab !== "livewall" && (
                <div className="flex items-center border-b border-outline-variant/10 shrink-0">
                  ...
                  >
                    ...
                    {t("app.alert_wall.logs")} ({displayLogCount})
                  </button>
                  <button
              ...
              {mainTab === "livewall" ? (
                <div className="LiveWallSidebar device-draggable-container flex-1 overflow-y-auto custom-scrollbar p-3 bg-surface-container-low/10 flex flex-col gap-2 relative">
                  ...
                  {cameraDevices
                    .filter((cam: any) => cam.type === "sunell")
                    .map((cam: any) => (
                      <div
                        key={cam.id}
                        draggable
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex flex-col min-w-0">
                            <span className="text-[11px] font-bold uppercase tracking-widest group-hover:text-secondary transition-colors truncate">
                              {cam.name || cam.cameraIp}
                            </span>
                            <span className="text-[9px] text-on-surface-variant/70 font-mono truncate">
                              {cam.cameraIp}
                            </span>
                          </div>
                          <span className="text-[9px] px-1.5 py-0.5 bg-surface-container-highest rounded text-on-surface-variant uppercase font-medium shrink-0">
                            sunell
                          </span>
                        </div>
                      </div>
                    ))}
                  {cameraDevices.filter((cam: any) => cam.type === "sunell")
                    .length === 0 && (
                    <div className="p-3 text-[10px] opacity-40 text-center italic">
                      Không có camera Sunell
                    </div>
                  )}
                  ...
                  <div className="relative p-3 flex items-center justify-between border-b border-outline-variant/10 shrink-0 bg-surface-container-lowest">
                    ...
                    <LogFilter
                      ...
                      onToggleAllEventTypes={(checked) => {
                        if (checked) {
                          setSelectedEventTypes(
                            eventTypes.map((item) => item.event_type),
                          );
                        } else {
                          setSelectedEventTypes([]);
                        }
                  ...
                  <div className="app-logs-container flex-1 overflow-y-auto custom-scrollbar p-0 bg-surface-container-low/10">
                    {displayLogs.length > 0 ? (
                      ...
                      border-b-2 border-transparent cursor-pointer"
                          >
                            {t("app.alert_wall.see_more_alerts")}
                          </button>
                        )}
                      </div>
                    ) : (
                      <div className="p-10 flex flex-col items-center justify-center opacity-20 gap-2 h-full text-center">
                        ...
                        <span className="text-[10px] uppercase font-bold tracking-widest">
                          {t("app.alert_wall.logs_queue_empty")}
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div className="device-draggable-container flex-1 overflow-y-auto custom-scrollbar p-3 bg-surface-container-low/10 flex flex-col gap-2 relative">
                  <div className="flex items-center justify-between sticky top-0 py-1 z-10 backdrop-blur-md mb-2 rounded-md px-1">
                    <span className="text-[9px] uppercase tracking-widest text-on-surface-variant opacity-70 font-bold">
                      {t("app.alert_wall.drag_to_assign")}
                    </span>
                    <button
                      onClick={() => {
                        console.log(devices);
                        const svmsDevices = Object.values(devices).flatMap(
                          (server) => {
                            if (!server.server) return [];
                            return (server.devices || []).map((dev) => ({
                              server_serial: server.server.serial,
                        ...
                        for (const dev of allDevices) {
                          const isAssigned = newGrids.some((g) =>
                            gridHasDevice(g, dev),
                          );
                  ...
                  {Object.values(devices).flatMap((server) => {
                    ...
                    const assignedGrids = grids.filter((g: any) =>
                      groupDevices.some((dev) => gridHasDevice(g, dev)),
                    );
                    return (
                      <div
                    ...
                    return (
                      <div key={ms.id} className="flex flex-col gap-1 mb-2">
                        ...
                        {mqttDevs.map((dev) => {
                          ...
                          const assignedGrids = grids.filter((g: any) =>
                            gridHasDevice(g, dragDevice),
                          );
                          const assignedText = assignedGrids
                            .map((g: any) => g.gridID + 1)
                            .join(", ");
                          const link = deviceCameraLinks.find(
                            (l) =>
                              l.mqttDeviceId === (dev as any).id ||
                              (l.devEui === dev.devEui && l.groupId === ms.id),
                          );
                          return (
                            <div
                              ...
                              draggable
                              title={
                                assignedGrids.length > 0
                                  ? `${t("app.alert_wall.assigned_to_grid")}${assignedText}`
                                  : undefined
                              }
                              onDragStart={(e) => {
                                e.dataTransfer.setData(
                            ...
                            >
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <div className="flex flex-col items-end gap-1 shrink-0">
                                  {link &&
                                    (() => {
                                      const linkedCam = cameraDevices.find(
                                        (c) => c.id === link.cameraId,
                                      );
                                      const camLabel =
                                        linkedCam?.name ||
                                        linkedCam?.cameraIp ||
                                        (link.cameraId
                                          ? link.cameraId.slice(-6)
                                          : "Default");
                                      return (
                                        <span className="text-[8px] px-1 py-0.5 rounded uppercase font-bold bg-cyan-500/20 text-cyan-500">
                                          📷 {camLabel}
                                        </span>
                                      );
                                    })()}
                  ...
                  {!mqttGroups.some(
                    (ms) => (mqttDevicesByServer[ms.id] || []).length > 0,
                  ) && (
                    <div className="p-4 flex flex-col items-center justify-center opacity-30 gap-2 text-center border border-dashed border-outline-variant/10 rounded">
                      <span className="text-[9px] uppercase font-bold tracking-widest">
                        {t("app.alert_wall.no_mqtt_devices")}
                      </span>
                    </div>
                  )}
                </div>
      ...
      {!isAlertWallFullscreen && (
        <footer className="app-footer shrink-0 h-6 bg-surface-container border-t border-outline-variant/10 flex items-center px-3 gap-4 text-[9px] font-mono select-none z-20">
          ...
          <div className="flex items-center gap-1.5">
            <span
              className={`w-1.5 h-1.5 rounded-full shrink-0 ${isConnected ? "bg-secondary" : "bg-red-500 animate-pulse"}`}
            />
            <span className="text-on-surface-variant/60 uppercase tracking-widest font-bold text-[8px]">
              Host
            </span>
          ...
          <button
            onClick={async () => {
              if (
                !window.confirm(
                  "Xóa toàn bộ lịch sử event? Hành động này không thể hoàn tác.",
                )
              )
                return;
              try {
