...
import type { EventTypeItem, LogData, ServerData, DeviceData, MqttDevice, MqttGroup } from '../types';
import { useTranslation } from 'react-i18next';
import { useSocketManager } from '../hooks/useSocketManager';
import { LogPopup } from './components/LogPopup';
import { SlidersHorizontal, Terminal, Check, Cpu, MonitorSmartphone, Settings, Monitor, Network, PanelRightOpen, PanelRightClose, Languages, LogOut, ChevronDown, MapPinned, Car, Tv, Trash2, AlertCircle } from 'lucide-react';
...
function LogFilter({
  ...
  const deviceList = useMemo(() => {
    ...
    const mqttDevs = Object.entries(mqttDevicesByServer || {}).flatMap(([serverId, devs]) => {
      return devs.map(d => ({
        name: d.deviceNickname || d.deviceInfo?.deviceNickname || d.deviceName || d.deviceProfileName || d.devEui || 'MQTT Device',
        ip: d.devEui || d.id || '',
        type: 'radar',
        index: 0,
        serverId: serverId,
                  ...
                  {Object.entries(groupedDevices).map(([serverKey, devs]) => {
                    ...
                    return (
                      <div key={serverKey} id={`device-group-${serverKey}`} className="flex flex-col gap-[1px] border-b border-outline-variant/5 pb-1 mb-0.5 last:border-0 last:pb-0 last:mb-0">
                        <button
                          onClick={() => {
                            const groupDevKeys = devs.map(d => `${d.serverId}_${d.ip}_${d.originalName || d.name}`);
          ...
          <div className="flex flex-col overflow-hidden h-full">
            {!isAlertWallFullscreen && (
              <div className={`appTabsBar flex items-center border-b border-outline-variant/10 shrink-0 ${isNarrow ? '' : 'px-6 gap-4'}`}>
                ...
                >
                  <AlertCircle className={`w-5 h-5 ${mainTab === 'alert' ? 'text-primary' : 'text-on-surface'}`} />
