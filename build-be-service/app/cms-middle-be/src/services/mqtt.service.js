...
function disconnectAllMqttDevices() {
  for (const id of Array.from(mqttClients.keys())) {
...
async function _appendEventLogs(entry, parsedBody, payload, events, log_source = 'milesight-radar') {
  ...
  for (const event of events) {
    ...
    const isCameraBound = eventCameraId && eventCameraId !== 'none';
    let snapshot = null;
    if (isCameraBound) {
      snapshot = await getSnapshotCached(eventCameraId);
...
function publishDownlink(mqttDeviceId, { applicationId, devEui, fPort, dataBase64, confirmed = false }) {
  // Disabled: the previous command/down topic was not the correct functional
  // contract for this application's Milesight integration.
  return {
    success: false,
    error: '[Milesight][Downlink] Unsupported: previous command/down topic was incorrect',
  };
}

function controlBuzzer(mqttDeviceId, { applicationId, devEui, enable, fPort = 85 }) {
  // Disabled together with downlink because the previous command contract was incorrect.
  return {
    success: false,
    error: '[Milesight][Buzzer] Unsupported: no verified command contract is configured',
  };
}
