 ...
 *        sdkPath: path.join(__dirname, '..', 'module', 'sunell'),
...
// ===== CHỤP SNAPSHOT TRẢ VỀ BASE64 =====

/**
 * Chụp 1 frame JPEG qua RTSP và trả về chuỗi base64.
 * Dùng cho tích hợp MQTT — không cần broadcast WebSocket.
 * @param {string} [rtspUrl] - URL RTSP (nếu không truyền, dùng config đã init)
 * @param {number} [timeoutMs=8000] - Timeout (ms)
 * @returns {Promise<string|null>} base64 string (không có prefix data:image) hoặc null nếu lỗi
 */
async function captureSnapshotBase64(rtspUrl, timeoutMs = 8000) {
    const url = rtspUrl || state.rtspUrl;
    if (!url) {
        console.log('[CameraModule] captureSnapshotBase64: RTSP URL chưa cấu hình');
        return null;
    }

    const tmpDir = state.snapshotDir || path.join(__dirname, '..', '..', 'snapshots');
    if (!fs.existsSync(tmpDir)) {
        fs.mkdirSync(tmpDir, { recursive: true });
    }

    const filename = `mqtt_snap_${Date.now()}.jpg`;
    const outputPath = path.join(tmpDir, filename);

    try {
        await captureSnapshot(url, outputPath, timeoutMs);
        const imgBytes = fs.readFileSync(outputPath);
        const base64 = imgBytes.toString('base64');

        // Xoá file tạm sau khi đã đọc base64
        try { fs.unlinkSync(outputPath); } catch (_) { /* ignore */ }

        log('IN', '[RTSP→Base64] OK', `${(imgBytes.length / 1024).toFixed(1)} KB`);
        return `data:image/jpeg;base64,${base64}`;
    } catch (err) {
        log('IN', '[RTSP→Base64] FAIL', err.message);
        // Xoá file lỗi nếu có
        try { fs.unlinkSync(outputPath); } catch (_) { /* ignore */ }
        return null;
    }
